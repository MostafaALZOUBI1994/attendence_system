...
class TeamContactScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return BaseScreen(
      titleKey: 'Team Contacts',
      child: BlocProvider(
        create: (ctx) => EmployeesCubit(ctx.read())..loadForMyDepartment(),
        child: BlocBuilder<EmployeesCubit, EmployeesState>(
          builder: (context, state) => state.when(
            initial: () => const SizedBox.shrink(),
            loading: () => const Center(child: CircularProgressIndicator()),
            error: (m) => Center(child: Padding(padding: const EdgeInsets.all(16), child: Text(m))),
...
class _EmployeesGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (employees.isEmpty) {
      return const Center(child: Padding(padding: EdgeInsets.all(24), child: Text('No team members found.')));
    }
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            CircleAvatar(radius: 35, backgroundImage: avatar, backgroundColor: lightGray,
            ...
            Text(title, maxLines: 2, textAlign: TextAlign.center, overflow: TextOverflow.ellipsis,
            ...
            Row(mainAxisAlignment: MainAxisAlignment.center, children: [
              ...
              Text(ext.isNotEmpty ? 'Ext: $ext' : '—', style: const TextStyle(fontSize: 12, color: secondaryColor)),
            ]),
          ],
        ),
      ),
