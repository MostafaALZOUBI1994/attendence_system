...
class _HRRequestScreenState extends State<HRRequestScreen>
  ...
  TimeOfDay _toTime = TimeOfDay.fromDateTime(DateTime.now().add(const Duration(hours: 1)));
                              ...
                              child: Form(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                        ...
                                        Expanded(
                                          child: _TimeCard(
                                            ...
                                            onTap: () async {
                                              await _pickTime(
                                                initial: _fromTime,
                                                onPicked: (t) => setState(() => _fromTime = t),
                                              );
                                            },
                                          ),
                                        ),
                                    ...
                                    _SectionTitle(_isLeaveRequest ? 'res'.tr() : 'crrRes'.tr()),
                                    const SizedBox(height: 8),
                                    TextFormField(
                                      ...
                                      validator: (v) =>
                                      (v == null || v.trim().isEmpty) ? 'plsEntRes'.tr() : null,
                                    ),
                                    const SizedBox(height: 18),
                                    _SectionTitle('attachments'.tr()),
                                    const SizedBox(height: 8),
                                    _attachment == null
                                        ? _AttachmentDrop(onPick: _pickAttachment)
                                        : _AttachmentPreview(
                                      file: _attachment!,
  ...
  void _blocListener(BuildContext context, ServicesState state) {
    state.maybeWhen(
      ...
      submissionFailure: (msg) => _dialog(context, DialogType.error, 'oops', msg),
      orElse: () {},
    );
  }
  ...
  Report? _findReportFor(DateTime date) {
    final st = context.read<ReportBloc>().state; // non-listening
    final list = st.whenOrNull(loaded: (all, filtered) => all) ?? const <Report>[];
    ...
    await showModalBottomSheet(
      ...
      builder: (_) => _BottomSheetWrap(
        child: SimpleDatePicker(
          selectedDate: init,
          allowedDates: allowed, // only these dates are shown/selectable
          onDateSelected: (d) async {
            await _applyAutoTimesForDate(d); // auto-fill times based on late/early
            onPicked(d);
          ...
          child: IgnorePointer(
            child: Opacity(
              ...
              child: Align(
                ...
                child: Icon(Icons.blur_on_rounded, size: 120, color: primaryColor),
              ),
            ),
          ),
        ),
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Text('${'available'.tr()}: ${leaveBalance.noOfHrsAvailable}',
            ...
            CircularPercentIndicator(
              ...
              center: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 4),
                  Text('hrs'.tr(), style: theme.textTheme.labelMedium?.copyWith(color: Colors.grey[600])),
                ],
              ),
    ...
    return Wrap(
      ...
      children: [
        for (final t in types)
          ChoiceChip(
            label: Text(locale == 'ar' ? t.permissionNameAR : t.permissionNameEN),
            ...
            labelStyle: TextStyle(
              ...
              fontWeight: selected == t.permissionCode ? FontWeight.w700 : FontWeight.w500,
            ),
            backgroundColor: Colors.white,
            shape: RoundedRectangleBorder(
              side: BorderSide(color: selected == t.permissionCode ? primaryColor : Colors.grey.shade300),
    ...
    return DottedBorderContainer(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text('noFileChs'.tr(), style: TextStyle(color: Colors.grey[700], fontWeight: FontWeight.w600)),
          const SizedBox(height: 2),
          Text('attachments'.tr(), style: TextStyle(color: Colors.grey[600], fontSize: 12)),
        ],
      ),
    );
  }
...
class _AttachmentPreview extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return ClipRRect(
      ...
      child: Stack(
        children: [
          AspectRatio(aspectRatio: 16 / 9, child: Image.file(file, fit: BoxFit.cover)),
          Positioned(
            top: 8,
            right: 8,
            child: Material(
...
class _TinyTips extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Text(text, style: TextStyle(fontSize: 12, color: Colors.grey[600], letterSpacing: 0.2));
  }
}
